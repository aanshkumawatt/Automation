...
class LNPAutomation:
    ...
    def generate_random_data(self):
        ...
        return {
            'portable_number': f"{random.randint(200, 999)}{random.randint(100, 999)}{random.randint(1000, 9999)}",
    ...
    async def validate_portable_number_and_proceed(self):
        ...
        try:
            ...
            for attempt in range(10):  # Try up to 10 different numbers
                logger.info(f"Attempt {attempt + 1}: Trying number {data['portable_number']}")
                
                # Find and clear the number input field
                number_input = self.page.locator('input[name="number2port"]')
                await number_input.clear()
                await number_input.fill(data['portable_number'])
                await self.page.wait_for_timeout(1000)  # Faster validation wait
                ...
                try:
                    ...
                    await self.page.wait_for_timeout(1500)  # Faster page load wait
                    ...
                    if requested_fields > 0:
                        logger.info(f"Successfully proceeded to next page with number {data['portable_number']}")
                        logger.info(f"Found {requested_fields} 'Requested' fields on the page")
                        return data['portable_number']
                    else:
                        # We're still on the same page, try next number
                        logger.info(f"Still on same page, trying next number...")
                        data = self.generate_random_data()
                        continue
                        
                except Exception as e:
                    logger.info(f"Next button click failed, trying next number: {str(e)}")
                    data = self.generate_random_data()
                    continue
            
            # If we get here, we couldn't proceed with any number
            logger.warning("Could not proceed to next page with any portable number")
            return data['portable_number']
            
        except Exception as e:
            logger.error(f"Portable number validation failed: {str(e)}")
            return self.generate_random_data()['portable_number']
    ...
    async def fill_first_form_page(self):
        ...
        try:
            ...
            portable_number = await self.validate_portable_number_and_proceed()
            logger.info(f"Using portable number: {portable_number}")
            self.test_data['portable_number'] = portable_number
    ...
    def generate_pdf_report(self):
        ...
        try:
            ...
            summary_data = [
                ...
                ['Portable Number Used', self.test_data.get('portable_number', 'N/A')]
            ]
            ...
            form_data = [
                ...
                ['Portable Number', self.test_data.get('portable_number', 'N/A'), '✓'],
